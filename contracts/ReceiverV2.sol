...
contract ReceiverV2 is Ownable2Step, SwapCallV2, ReentrancyGuard, IButterReceiver {
    ...
    function _checkBalance(
        ...
        uint256 balance = _getBalance(_srcToken, self);
        if (balance < _amount) revert Errors.RECEIVE_LOW();
        nativeBalance = self.balance;
    ...
    function remoteCall(
        ...
        uint256 _amount,
        bytes calldata _callbackData
    ) external returns (address target, uint256 callAmount, address receiver) {
        if (msg.sender != address(this)) revert Errors.SELF_ONLY();
        (callAmount, receiver, target) = _callBack(_amount, _callToken, _callbackData);
    }
