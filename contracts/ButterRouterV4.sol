...
import "./interface/IButterRouterV4.sol";
import "./abstract/FeeManager.sol";

contract ButterRouterV4 is SwapCallV2, FeeManager, ReentrancyGuard, IButterRouterV4 {
    ...
    ) external payable override nonReentrant ensure(_deadline) returns (bytes32 orderId) {
    ...
    function swapAndCall(
        ...
        bytes calldata _permitData,
        bytes calldata _feeData
    ) external payable override ensure(_deadline) nonReentrant {
        SwapTemp memory swapTemp;
        swapTemp.initiator = _initiator;
        swapTemp.srcToken = _srcToken;
        swapTemp.srcAmount = _amount;
